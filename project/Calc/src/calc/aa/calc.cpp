...
void Calc::SocketCommunication()
{
    while (m_running) {
        ...
        if (select_result > 0) {
            ...
            if (sent_bytes != static_cast<ssize_t>(combinedBuffer.size())) {
                ...
                if (!ReconnectToServer()) {
                    m_logger.LogError() << "Failed to reconnect, exiting communication loop";
                    break;
                }
                continue;
            }
        } else if (select_result == 0) {
            m_logger.LogError() << "Send timeout";
            if (!ReconnectToServer()) {
                m_logger.LogError() << "Failed to reconnect, exiting communication loop";
                break;
            }
            ...
            m_logger.LogError() << "Select error: " << strerror(errno);
            if (!ReconnectToServer()) {
                m_logger.LogError() << "Failed to reconnect, exiting communication loop";
                break;
            }
        ...
        if (select_result > 0) {
            ...
            m_logger.LogError() << "Receive timeout";
            if (!ReconnectToServer()) {
                m_logger.LogError() << "Failed to reconnect, exiting communication loop";
                break;
            }
...
// Field RField Notify 처리 함수
// // RawData RField Notify 처리 함수
